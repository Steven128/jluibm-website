...
//创建报名时间表
$join = "CREATE TABLE join_date( " .
    "date DATE NOT NULL COMMENT '注册时间'," .
    "number VARCHAR(8) NOT NULL PRIMARY KEY COMMENT '学号'," .
    ") ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 COMMENT '报名日期表';";
$retval = mysqli_query($infodb, $join);
if (!$retval) {
    die('数据表join_date创建失败<br><br>' . mysqli_error($infodb));
}
echo "数据表join_date创建成功<br><br>";
