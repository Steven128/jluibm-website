...
(function(factory) {
    ...
    _.support = {};
    //css3 event
    function prefixEvent(t) { if (t) { var e = document.createElement("div"),
                n = { Webkit: "webkit", Moz: "", O: "o" }; for (var i in n)
                if (void 0 != e.style[i + "TransitionProperty"]) return n[i] + String.fromCharCode(t[0].charCodeAt() - 32) + t.slice(1) } }

    //css3 style浏览器兼容
    _.prefixStyle = function(style, css) {
        var el = document.createElement('div')
        var vendors = 'webkitT,t,MozT,msT,OT'.split(','),
            svendors = [],
            prefix, split = '-',
            _style = style;
    ...
    _.animateFrame = function(el, firstFrame, lastFrame, frameGapTime, isGoToFirstBoo, isLoopBoo, loopTimes, loopGapTime, callBackFun, stepFuc, waitFrame, waitTime) {
        var plugin = el.data("plugin"),
            frameClass, $list,
            isInit = plugin ? true : false,
            isPlay = plugin ? plugin.getPlayState() : false;
        if (isPlay) {
            return;
        }
        if (!isInit) {
    ...
    _.cropImage = function(opts) {
        ...
        var G = { preview: null, moveX: 0, moveY: 0, scale: 1, rotate: 0, ratio: opts.devicePixelRatio },
            ...
            _x, _y, _scale, offset = $(canvas).offset(),
            $bindPreview = opts.bindPreview || $('<img/>'),
            canMove;
        ...
        function getCropInfo() {
            var iWidth = G.preview.naturalWidth,
                iHeight = G.preview.naturalHeight,
                dWidth, dHeight, x = 0,
                y = 0,
                scale;
    ...
    _.animateGroup = function(opts) {
        ...
        var animArr = opts.group,
            reset, duration = opts.duration + (opts.gap || 0),
            curEl = animArr.eq(0),
